...
async def test_send_packet_retry_success(hass: HomeAssistant):
    ...
    with patch.object(device, "_connect", new_callable=AsyncMock) as mock_connect, \
...
async def test_send_packet_timeout_retry(hass: HomeAssistant):
    """Test that TimeoutError triggers a retry."""
    device = BoksBluetoothDevice(hass, "AA:BB:CC:DD:EE:FF", "12345678")
    ...
    # Mock connect to always restore client
    async def side_effect_connect(*args, **kwargs):
        device._client = mock_client

    with patch.object(device, "_connect", new_callable=AsyncMock, side_effect=side_effect_connect):
        # We expect a timeout waiting for response
        # Using a very short timeout for test speed
        # It should retry once, so 2 attempts total, and fail on the 2nd
        ...
        # Should have called write twice (retry on timeout)
        assert mock_client.write_gatt_char.call_count == 2
