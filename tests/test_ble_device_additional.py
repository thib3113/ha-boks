...
async def test_get_logs_success(hass: HomeAssistant):
    ...
    device._client.disconnect = AsyncMock() # Mock disconnect
    
    # Mock send_packet for request
    with patch.object(device, "_send_packet", new_callable=AsyncMock), \
        ...
        device._send_packet.side_effect = side_effect_send_packet
