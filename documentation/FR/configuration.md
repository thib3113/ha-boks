...
*   **1. Code Maître Uniquement (Requis pour le Fonctionnement de Base)**
    ...
    *   **Fonctionnalités Activées** :
        ...
        *   **Journalisation des Événements** : Recevez les événements opérationnels de base de votre Boks (par exemple, porte ouverte/fermée, tentatives de code valides/invalides) via l'entité `event.boks_logs`.
        *   **Liste de Tâches (Fonctionnalité de Base)** : Une entité `todo.suivi_boks` sera créée. Vous pouvez l'utiliser pour suivre les colis, mais vous devrez gérer manuellement (créer et associer) tous les codes PIN. L'intégration tentera toujours de valider et de marquer les tâches comme terminées si elle détecte un code associé manuellement dans ses journaux et émettra des événements `boks_parcel_completed`.
...
*   **2. Clé de Configuration ou Clé Maître (Optionnel, Recommandé pour les Fonctionnalités Avancées)**
    ...
    *   **Fonctionnalités Activées** :
        ...
        *   **Intégration Améliorée de la Liste de Tâches (Gestion des Colis)** : L'entité `todo.suivi_boks` offre une fonctionnalité complète.
            *   **Génération Automatique de PIN** : Lorsque vous ajoutez un élément à la liste `todo.suivi_boks` avec une description (par exemple, "Colis Amazon"), l'intégration générera automatiquement un code PIN unique associé à cette entrée de colis.
